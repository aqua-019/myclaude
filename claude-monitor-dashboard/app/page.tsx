...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const ws = new WebSocket('wss://myclaude-production.up.railway.app');
    ...
    ws.onmessage = (event) => {
      ...
      switch (message.type) {
        case 'initial_data':
          ...
          console.log('Loading initial data from database:', message.data);
          
          // Load sessions
          if (message.data.sessions && message.data.sessions.length > 0) {
            const loadedSessions = message.data.sessions.map((s: any) => ({
              id: s.session_id,
          ...
          // Load tokens into history
          if (message.data.tokens && message.data.tokens.length > 0) {
            const loadedTokens = message.data.tokens.map((t: any) => ({
            ...
            setTokenHistory(loadedTokens);
            
            // Update total tokens in metrics
            const totalTokens = loadedTokens.reduce((sum: number, t: any) => sum + t.total, 0);
            setMetrics(prev => ({ ...prev, totalTokens }));
          }
          
          // Load tasks
          if (message.data.tasks && message.data.tasks.length > 0) {
            const loadedTasks = message.data.tasks.map((t: any) => ({
              id: t.task_id,
              sessionId: t.session_id,
